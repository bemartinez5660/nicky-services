...
class BathroomsNumber extends StatefulWidget {
  const BathroomsNumber({super.key, required this.input});
  final Input input;

  @override
  State<BathroomsNumber> createState() => _BathroomsNumberState();
}

class _BathroomsNumberState extends State<BathroomsNumber> {
  int _selectedItem = 0;
  ...
  void initState() {
    ...
    widget.input.userserviceconfigSet!.create!.removeWhere((element) =>
        element.parameter!.connect!.parameter!.exact == "bathrooms");
    widget.input.userserviceconfigSet!.create!.add(Create(
        parameter: Parameter(
            connect: ParameterConnect(parameter: Id(exact: "bathrooms"))),
